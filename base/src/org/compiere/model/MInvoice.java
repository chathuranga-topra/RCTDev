...
public class MInvoice extends X_C_Invoice implements DocAction
{
	 ...
	 *	@return Invoice
	 */
	public static MInvoice copyFrom (MInvoice from, String DocNo, Timestamp dateDoc, Timestamp dateAcct,
		int C_DocTypeTarget_ID, boolean isSOTrx, boolean counter,
		String trxName, boolean setOrder)
	{
		MInvoice to = new MInvoice (from.getCtx(), 0, trxName);
		PO.copyValues (from, to, from.getAD_Client_ID(), from.getAD_Org_ID());
		to.set_ValueNoCheck ("C_Invoice_ID", I_ZERO);
		to.set_ValueNoCheck ("DocumentNo", DocNo);
		//
		to.setDocStatus (DOCSTATUS_Drafted);		//	Draft
		to.setDocAction(DOCACTION_Complete);
		//
...
//		MOrgInfo counterOrgInfo = MOrgInfo.get(getCtx(), counterAD_Org_ID);
	...
	public boolean reverseCorrectIt()
	{
		...
		//	Deep Copy
		MInvoice reversal = copyFrom (this, this.getDocumentNo() + "-RE" , getDateInvoiced(), getDateAcct(),
