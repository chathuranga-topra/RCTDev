...
import org.rct.modelValidator.ParkingACR;
...
public class InvoiceGenerate extends SvrProcess
{
	...
	private void createLine (MOrder order, MInOut ship, MInOutLine sLine)
	{
			...
			|| m_ship.getM_InOut_ID() != ship.getM_InOut_ID())
		{
			...
			//description stand for container number
			//line.setDescription("ABCD DISSA");
	...
	private void completeInvoice()
	{
		if (m_invoice != null)
		{
			//set container numbers for invoice lines
			this.setContainerNumbers(m_invoice);
			
			//get non invoiced order lines
			//this.getNonInvoicedOrderLines(m_invoice);
			...
			m_invoice.saveEx();
			//Parking after customs release
			this.setParkingACR(m_invoice);
	...
	private void setContainerNumbers(MInvoice mInvoice){
		...
		//get tax category
		BpartnerTaxCategory bpartnerTaxCategory = new BpartnerTaxCategory();
		int tax_id = bpartnerTaxCategory.getTaxGategory(mInvoice.get_TrxName());
		
		//SET DEFAULT BUSINESS PARTNER PAY RULE
		this.setBpartnerDefaultPayRule(mInvoice);
		
		for(MInvoiceLine line : lines){
			
			orderline = line.getC_OrderLine();
			order = orderline.getC_Order();
			
			MOrderLine morderline = new MOrderLine(getCtx(), orderline.getC_OrderLine_ID(), get_TrxName());
			
			if(order.getDescription()!=null){
				line.setDescription(order.getDescription());
				//set container size to invoice line  : Purpose is to generate reports
				line.set_CustomColumn("M_Pricelist_ID", order.getM_PriceList_ID());
			}
			
			if(morderline.get_Value("n_starttime")!=null){
				line.set_ValueOfColumn("start_time", morderline.get_Value("n_starttime"));
			}
			
			if(morderline.get_Value("n_endtime")!=null){
				line.set_ValueOfColumn("end_time", morderline.get_Value("n_endtime"));
			}
			
			//setup vat/svat
			if(order.getC_BPartner().isTaxExempt())
				line.setC_Tax_ID(tax_id);
			
			//COPY SO seal charge number in to invoice
			if(morderline.getM_Product_ID() == tpUtility.getSeal_Charge_id() || morderline.getM_Product_ID() == tpUtility.getSealNumber()){
				int seal_no = morderline.get_ValueAsInt("c_seal_no");
				if(seal_no > 0)
				line.set_CustomColumn("c_seal_no", morderline.get_ValueAsInt("c_seal_no"));
			}
	...
	//Invoice should pick default business partner's payment rule
	private void setBpartnerDefaultPayRule(MInvoice mInvoice){
		
		String pr = mInvoice.getC_BPartner().getPaymentRule();
		if(pr== null || pr.length() == 0)
			pr = X_C_Invoice.PAYMENTRULE_Cash;
		
		mInvoice.setPaymentRule(pr);
		mInvoice.save();
	}
	
	private void setParkingACR(MInvoice mInvoice){
		new ParkingACR().setParkingACR(mInvoice);
	}
}