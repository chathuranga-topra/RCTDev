...
public class VGenPanel extends CPanel implements ActionListener, ChangeListener, TableModelListener, ASyncProcess
{
	...
	{
		//Invoice Gen Process Update By Chathuranga
		if(pi.getAD_Process_ID() == 134){
			...
			if(pi.getIDs() != null){
				//Clear cusdeck
				((VInvoiceGen) genForm).setfCusdec("");;
				tabbedPane.setSelectedIndex(0);
				//Open tax invoice window
				int AD_Window_ID = 1000009;		// TaxInvoice Window ID
				String ColumnName = "C_INVOICE_ID";
				int Record_ID = pi.getIDs()[0];
			...
			if (ADialog.ask(m_WindowNo, this, genForm.getAskPrintMsg()))
			{
				...
				do
				{
					...
					//Update is printed in MInout
					if(retValue == ADialogDialog.A_OK  &&  pi.getAD_Process_ID() == 199){
						
						String sql = "UPDATE M_InOut SET ISPRINTED = 'Y' WHERE M_InOut_ID = " +Record_ID ;
	...
	private void setShipmentPrintOK(){
